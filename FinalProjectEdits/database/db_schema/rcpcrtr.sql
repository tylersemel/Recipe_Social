...
CREATE TABLE IF NOT EXISTS `follow` (
  `fol_user_source` int(11) unsigned NOT NULL,
  `fol_user_target` int(11) unsigned NOT NULL,
  PRIMARY KEY (`fol_user_source`,`fol_user_target`),
  KEY `FK_FOL_SRC` (`fol_user_source`),
  KEY `FK_FOL_TGT` (`fol_user_target`),
  CONSTRAINT `FK_FOL_SRC` FOREIGN KEY (`fol_user_source`) REFERENCES `user` (`usr_id`) ON DELETE CASCADE ON UPDATE NO ACTION,
  CONSTRAINT `FK_FOL_TGT` FOREIGN KEY (`fol_user_source`) REFERENCES `user` (`usr_id`) ON DELETE CASCADE ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
