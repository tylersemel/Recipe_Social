...
function getSavedRecipesFolder(userId) {
    return db.query("SELECT * FROM folder WHERE fdr_owner=? AND fdr_name='Saved Recipes'", [userId]).then(({results}) => {
        let folder_id = results[0].fdr_id;

        return getRecipesInFolder(userId, folder_id);

    })
}

// //Gets all the recipes by its tag
// async function getRecipesByTag(tagId) { 
//     return db.query("SELECT * FROM recipe_tag WHERE rct_tag_id=?", [tagId]).then(async ({results}) => {
//         let recipes = [];

//         console.log(results);
        
//         for (const result of results) {
//             await getRecipesByUser(result.rct_rcp_id).then(recipe => {
//                 console.log(recipe);
//                 recipes.push(recipe);
//             });
...
//get recipes from a user by the selected tag
async function getRecipesByUserByTag(userId, tagId) {
     return db.query("SELECT rct_rcp_id FROM recipe_tag WHERE rct_tag_id=?", tagId).then(async ({results}) => {
        let recipeIds = results.map(id => id.rct_rcp_id);
        console.log("recipes id", recipeIds);
        let taggedRecipes = [];

        //get all recipes beloging to user
        let userRecipes = ( await getRecipesByUser(userId));

        //check if the user recipe ids match the recipe id found
        for (let i = 0; i < userRecipes.length; i++) {
            console.log("userRecipe line 214", userRecipes[i]);
            for (const recipeId of recipeIds) {
                console.log("recipeId", recipeId);
                console.log("user recipe id", userRecipes[i].id);
                if (userRecipes[i].id == recipeId){
                    taggedRecipes.push( userRecipes[i] ); 
                }  
            }
        }  
...
//get all tags
function getTags() {
    return db.query("SELECT * FROM tags").then(async ({results}) => {
        let ids = results.map(id => id.tag_id);
        let tags = [];
        for (const id of ids) {
            tags.push( await getTagsById(id));
        }
        return tags;
    });
}

function getTagsById(tagId) {
    return db.query("SELECT * FROM tags WHERE tag_id=?", 
            [tagId]).then(({results}) => {
                let tag = results.map(tag=> new Tag(tag));
                console.log("tags id", tag[0]);
                return tag;   
            });    
}
...
function checkAccess(id, recipeid) {
    return db.query("SELECT * FROM user_recipe WHERE usp_usr_id=? AND usp_rcp_id=?", [id, recipeid]).then(({results}) => {
...
module.exports = {
    ...
    getRecipesByUserByTag: getRecipesByUserByTag,
    getTags: getTags,
    getTagsById: getTagsById,
    getSavedRecipesFolder: getSavedRecipesFolder,
